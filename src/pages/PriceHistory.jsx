        ...
        const fetchApartments = async () => {
            try {
                ...
                const transformedData = json.map((item, index) => ({
                    ...
                    flatPriceM2: calcPriceM2(
                        (item.history_price ? getLatestPrice(item.history_price) : item.fm_price) || 0,
                        item.fm_apartmentArea || 0,
                        item.fm_flatNumber
                    ),
                    flatPrice: item.history_price ? getLatestPrice(item.history_price) : item.fm_price,
                    roomCount: item.fm_rooms,
                    status: item.fm_status,
                    history_price: item.history_price || [], // <-- dodane
                    ...
                    <table className="w-full table-auto border-collapse text-white font-poppins">
                        <thead className="bg-[#1E1E1E]">
                            <tr className="h-[100px] border-t border-b border-[#3E3E3E] uppercase text-[18px]">
                                ...
                                <th className="text-center px-4">Cena / Data aktualizacji</th>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <td className="text-center px-4">
                                            {item.history_price?.length > 0 ? (
                                                <div className="flex flex-col items-center">
                                                    {item.history_price.map((h, idx) => (
                                                        <span key={idx}>
                                                            {h.h_date} – {h.h_price} zł
                                                        </span>
                                                    ))}
                                                </div>
                                            ) : (
