...
const NavBar = () => {
    ...
    return (
        <header
            className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-[#1E1E1E]/85 h-[80px]' : 'h-[100px] md:h-[200px]'
                }`}
        >
            <div className="container max-w-[1596px] mx-auto h-full px-6 flex items-center justify-between">
                ...
                <div className="flex items-center">
                    <img
                        src={logo}
                        alt="Logo"
                        className={`transition-all duration-300 ${isScrolled ? 'h-[60px]' : 'h-[60px] lg:h-[100px]'}`}
                    />
                </div>

                {/* Navigation Section */}
                <nav className="hidden xl:flex items-center space-x-8 text-white text-[16px]">
                    {['INWESTYCJA', 'APARTAMENTY', 'UDOGODNIENIA', 'LOKALIZACJA', 'GALERIA', 'O DEWELOPERZE', 'KONTAKT'].map((item, index) => (
                ...
                <div className="hidden xl:block font-poppins text-white font-light">
                    <Link to={`tel:${Config.contact.phone}`}>{Config.contact.phoneFormated}</Link>
                </div>
                {/* Mobile Menu Button */}
                <button
